...
typedef struct ms_attestMonitor_t {
...
static sgx_status_t SGX_CDECL sgx_attestMonitor(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_attestMonitor_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_attestMonitor_t* ms = SGX_CAST(ms_attestMonitor_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	attestMonitor(ms->ms_monitorSocket);
...
SGX_EXTERNC const struct {
	...
	4,
	{
		...
		{(void*)(uintptr_t)sgx_attestMonitor, 0},
