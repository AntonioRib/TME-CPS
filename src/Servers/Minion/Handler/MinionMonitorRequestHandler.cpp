...
bool MinionMonitorRequestHandler::deployApp(string appId) {
    ...
    string deployString = "sudo docker run --name "+appId+" -d -p " + std::to_string(port) + ":" + std::to_string(6379) + " -i -t redis";
    ...
    int sizeDeployStream = asprintf(&deployContainerArgsStream, "sudo docker run --name %s -d -p %d:%d -i -t redis", appId.c_str(), port, 6379, appId.c_str());
